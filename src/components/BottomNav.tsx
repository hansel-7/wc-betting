...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-forest-900/95 backdrop-blur-sm border-t border-forest-700/50 z-50">
      <div className="max-w-md mx-auto flex justify-around items-end h-16 px-2">
        {/* Home */}
        <NavLink href="/" active={isActive("/")} label="Home" icon={HomeIcon} />

        {/* Social */}
        <NavLink href="/social" active={isActive("/social")} label="Social" icon={SocialIcon} />

        {/* Bet (center, raised) */}
        <Link href="/bet" className="flex flex-col items-center -mt-4">
          <div className={cn(
        ...
        {/* Leaderboard */}
        <NavLink href="/leaderboard" active={isActive("/leaderboard")} label="Ranks" icon={TrophyIcon} />

        {/* Profile */}
        <NavLink href="/profile" active={isActive("/profile")} label="Profile" icon={ProfileIcon} />
      </div>
    </nav>
  );
}
