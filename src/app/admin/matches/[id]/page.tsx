...
export default async function AdminMatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="px-4 py-4">
      <div className="mb-6">
        <p className="text-[10px] text-forest-500 mb-1">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-forest-500">vs</span>
          <span className="text-lg font-bold">{match.away_team}</span>
          <span className="text-2xl">{match.away_flag}</span>
        </div>
        <p className="text-xs text-forest-400">{formatMatchTime(match.match_time)} · {match.venue}</p>
      </div>

      {/* Pool Stats */}
      <div className="bg-forest-800/60 rounded-xl p-4 mb-6 border border-forest-700/20">
        <h3 className="text-xs text-forest-400 mb-2">Pool Statistics</h3>
        <div className="grid grid-cols-3 gap-3 text-center text-sm">
          <div>
            <p className="font-medium text-blue-400">{formatPoints(totalHome)}</p>
            <p className="text-[10px] text-forest-500">on {match.home_team}</p>
          </div>
          <div>
            <p className="font-medium">{betCount}</p>
            <p className="text-[10px] text-forest-500">total bets</p>
          </div>
          <div>
            <p className="font-medium text-orange-400">{formatPoints(totalAway)}</p>
            <p className="text-[10px] text-forest-500">on {match.away_team}</p>
          </div>
        </div>
      </div>
